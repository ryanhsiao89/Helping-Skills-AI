...
# --- 🌟 本研究專屬白名單 (Whitelist) - Email 已全部更新為小寫 🌟 ---
WHITELIST = {
    ...
    if st.session_state.generated_otp:
        ...
        if st.button("🚀 驗證並進入演練"):
            if user_otp == st.session_state.generated_otp:
                ...
                # 🌟 已經將模型改為穩定高乘載的 gemini-1.5-flash-latest
                genai.configure(api_key=st.session_state.api_key)
                model = genai.GenerativeModel(model_name="gemini-1.5-flash-latest", generation_config=GenerationConfig(temperature=0.0))
    ...
    if not st.session_state.supervisor_feedback:
        ...
        with st.spinner("👨‍🏫 臨床督導正在審閱你的對話紀錄，進行 5+1 技巧評分... (約需 10-20 秒)"):
            try:
                ...
                # 🌟 已經將督導模型也改為穩定高乘載的 gemini-1.5-flash-latest
                supervisor_model = genai.GenerativeModel(model_name="gemini-1.5-flash-latest", generation_config=GenerationConfig(temperature=0.0))
